...
public class IntegerToRoman
{
    ...
    {
        var numbers = new [] { 1, 5, 10, 50, 100, 500, 1000 };
        var letters = new [] { 'I', 'V', 'X', 'L', 'C', 'D', 'M' };
        ...
        while (num > 0)
        {
            ...
            if (index > 1 && index % 2 == 0 && num - numbers[index] + numbers[index - 2] >=0)
            {
                num = num - numbers[index] + numbers[index - 2];
                builder.Append(letters[index - 2]);
                builder.Append(letters[index]);
                continue;
            }
            
            if (index > 0 && index % 2 == 1 && num - numbers[index] + numbers[index - 1] >=0)
            {
                num = num - numbers[index] + numbers[index - 1];
                builder.Append(letters[index - 1]);
                builder.Append(letters[index]);
                continue;
            }
